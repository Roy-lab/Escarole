...
Framework::startClustering(const char* aDir)
{
	...
	// No longer using the file produced by this --  allspecies_clusterassign_brk.txt
...
/*
 *  DC updated to remove clusterassign_multspecies file
 *
 */
int
Framework::generateData(const char* outputDir)
{
	...
	for(map<string,CLUSTERSET*>::iterator sIter=extantSpeciesSet.begin();sIter!=extantSpeciesSet.end();sIter++)
	{
		...
		// write headers to oFile and cFile
		// TODO -- oFile header
		//cFile << "Gene\t" << sIter->first << endl;
	}
